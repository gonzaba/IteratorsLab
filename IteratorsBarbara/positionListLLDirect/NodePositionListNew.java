...
public class NodePositionListNew<T> implements PositionList<T> {

	private DNode<T> header, trailer; 
	private int count; 
	private PositionListIteratorMaker<T> iterMaker;
	public NodePositionListNew() {
		header = new DNode<T>(null, null, null, this); 
		trailer = new DNode<T>(null, header, null, this);
		header.setNext(trailer); 
		count = 0; 
		iterMaker = new ForwardPositionListIteratorMaker<T>();
	}
	public NodePositionListNew(PositionListIteratorMaker<T> iterMaker) {
		header = new DNode<T>(null, null, null, this); 
		trailer = new DNode<T>(null, header, null, this);
		header.setNext(trailer); 
		count = 0; 
		this.iterMaker = iterMaker;
	}
	...
	public Iterator<T> iterator() {
		return iterMaker.makeIterator(this);
	}
	...
	private static class DNode<T2> implements Position<T2> {
        ...
        private NodePositionListNew<T2> myList;     // list it belongs to
        
        public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionListNew<T2> list) { 
        	element = e; 
        	next = n; 
        	prev = p; 
        	myList = list; 
		...
		public void setList(NodePositionListNew<T2> n) { 
			myList = n; 
		}
		public NodePositionListNew<T2> getList() { 
			return myList; 
		}
